...
public class StringtoInteger {
     ...
     */
    private static int myAtoiMySolution(String s) {
        int answer=0;
        char arr[]=s.toCharArray();
        char sign='+';
        answer=convertCharToInt(arr,0,arr.length-1,answer,sign);
        return answer;
    }

    private static int convertCharToInt(char[] arr, int i, int j,int answer,char sign) {
        if(i<=j){
            if(arr[i]==' ' || Character.isLetter(arr[i]))
                return convertCharToInt(arr,i+1,j,answer,sign);
            else if(arr[i]=='-') {
